...
function mintModal({
  ...
  const calculatePrice = () => {
    var price = parseFloat(nftAmount*0.005)
    price = price.toFixed(4)
    setAmountDue(price)
  }

  const checkInput = () => {
    (!isNaN(nftAmount) && Number.isInteger(parseFloat(nftAmount)) && nftAmount > 0 && nftAmount < 100000000000000) ? (
      setInputError('')
    ):(
      setInputError('Has to be an Integar (e.g. 1, 2, 10, etc)')
    )
  }

  const sendTransaction = (_nftAmount) => {
        mint(_nftAmount)
        setIsLoading(true)
  ...
  const signUpModalPage = () => {
    ...
    if (isLoading == true){
      ...
      return (
        <div
          className="bg-white w-11/12 max-w-md h-3/6 flex items-center justify-center"
        >
              <BounceLoader color='green'/>
              <p1 className='ml-2'> Minting.... </p1>
        </div>
      )
    }
    else {
              ...
              >
                ...
                {etherscanLink && (
                  <Link href={`${etherscanLink}`} >
                    <a className='w-full h-full flex items-center justify-center text-green-500 text-2xl mt-[20px] font-bold cursor-pointer mb-2'
                    target='_blank'>
                      Transaction Receipt
                    </a>
                  </Link>
          ...
          default:
            return (
              ...
              >
                ...
                {
                  <div className="px-4 mb-4">
                    <h2 className="text-3xl font-medium">How many NFT? </h2>
                      ...
                      <p className="mt-2 w-10/12 max-w-6xl max-w-full mx-auto text-gray-800 text-base">
                        {nftAmount && nftAmount>0 ? (`Amount Due: ${AmountDue} ETH`):(`0 ETH`)}
                      </p>
                      <p className="mt-2 w-10/12 max-w-6xl max-w-full mx-auto text-gray-800 text-base">
                        {currentAccount ? (`Mint Address: ${currentAccount}`):(``)}
                      </p>
                  </div>
                }
                <div
